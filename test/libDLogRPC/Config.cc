...
class ConfigTest : public ::testing::Test {
  public:
        ...
        : tmpdir(Storage::FilesystemUtil::tmpnam())
    {
        ...
        if (write(fd, file, downCast<uint32_t>(strlen(file))) == -1)
...
TEST_F(ConfigTest, readWithoutDefault) {
    Config config;
    config.set("k", "v");
...
TEST_F(ConfigTest, readWithDefault) {
    Config config;
    config.set("k", "v");
