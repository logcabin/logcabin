...
// class DumbFilesystemStorageModule

DumbFilesystemStorageModule::DumbFilesystemStorageModule(
                                    const std::string& path)
    : path(path)
{
    if (mkdir(path.c_str(), 0755) != 0) {
        if (errno != EEXIST) {
            PANIC("Failed to create directory for DumbFilesystemStorageModule:"
...
std::vector<LogId>
DumbFilesystemStorageModule::getLogs()
{
...
void
DumbFilesystemStorageModule::deleteLog(LogId logId,
...
std::string
DumbFilesystemStorageModule::getLogPath(LogId logId) const
{
    return format("%s/%016lx", path.c_str(), logId);
}

// class DumbFilesystemLog

DumbFilesystemLog::DumbFilesystemLog(LogId logId, const std::string& path)
    : Log(logId)
    , path(path)
    , headId(NO_ENTRY_ID)
    , entries()
{
    if (mkdir(path.c_str(), 0755) != 0) {
        if (errno != EEXIST) {
            PANIC("Failed to create directory for DumbFilesystemLog:"
                  " mkdir(%s) failed: %s", path.c_str(), strerror(errno));
        }
    }
...
std::deque<LogEntry>
DumbFilesystemLog::readFrom(EntryId start)
{
...
std::string
DumbFilesystemLog::getEntryPath(EntryId entryId) const
{
    return format("%s/%016lx", path.c_str(), entryId);
}

void
DumbFilesystemLog::read(EntryId entryId)
{
...
void
DumbFilesystemLog::write(const LogEntry& entry)
{
    ProtoBuf::DumbFilesystem::LogEntry contents;
