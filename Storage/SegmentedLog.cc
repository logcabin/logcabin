...
////////// SegmentedLog::Sync //////////
SegmentedLog::Sync::Sync(uint64_t lastIndex, Server::Globals*& globals)
    : Log::Sync(lastIndex)
    , fds()
    , writes()
    , globals(globals)
        ...
        switch (it->second) {
            case Op::FDATASYNC:
                if (lastDataSynced != it->first) {
                    ERROR("fdatasync after %lu bytes with %lu workers active "
                          "[Dispatch %lu, Raft %lu (%lu append %lu), SM %lu, Reply %lu]",
                          wrote,
                          globals->raft->activeWorkers.load(),
                          globals->raft->dispatchQueue.load(),
                          globals->raft->workersRaft.load(),
                          globals->raft->workersRaftPreAppend.load(),
                          globals->raft->workersRaftPostAppend.load(),
                          globals->raft->workersSM.load(),
                          globals->raft->workersReply.load()
                          );
                    FS::fdatasync(f);
...
SegmentedLog::SegmentedLog(const FS::File& parentDir)
    ...
    , currentSync(new SegmentedLog::Sync(0, globals))
    , segmentPreparer()
    , globals(NULL)
{
...
SegmentedLog::takeSync()
{
    std::unique_ptr<SegmentedLog::Sync> other(
            new SegmentedLog::Sync(getLastLogIndex(), globals));
