 ...
 */
#define OPEN_SEGMENT_FORMAT "open-%llu"
    ...
    for (auto it = segments.begin(); it != segments.end(); ++it) {
        ...
        if (keep) {
            ...
            if (!result.second) {
                ...
                PANIC("Two segments contain entry %llu: %s and %s",
                      startIndex,
    ...
    for (auto it = entries.begin(); it != entries.end(); ++it) {
        ...
        if (openSegment->bytes > sizeof(SegmentHeader) &&
            ...
            NOTICE("Rolling over to new head segment: trying to append new "
                   "entry that is %llu bytes long, but open segment is already "
                   "%llu of %llu bytes large",
                   buf.getLength(),
                   openSegment->bytes,
                   MAX_SEGMENT_SIZE);
        ...
        if (buf.getLength() > MAX_SEGMENT_SIZE) {
            WARNING("Trying to append an entry of %llu bytes when the maximum "
                    "segment size is %llu bytes. Placing this entry in its own "
...
SegmentedLog::getEntry(uint64_t index) const
{
    if (index < getLogStartIndex() ||
        ...
        PANIC("Attempted to access entry %llu outside of log "
              "(start index is %llu, last index is %llu)",
...
SegmentedLog::truncatePrefix(uint64_t newStartIndex)
{
    ...
    NOTICE("Truncating log to start at index %llu (was %llu)",
    ...
    while (!segmentsByStartIndex.empty()) {
        ...
        if (logStartIndex <= segment.endIndex)
            break;
        NOTICE("Deleting unneeded segment %s (its end index is %llu)",
               segment.filename.c_str(),
               segment.endIndex);
...
SegmentedLog::truncateSuffix(uint64_t newEndIndex)
{
    ...
    NOTICE("Truncating log to end at index %llu (was %llu)",
...
SegmentedLog::updateMetadata()
{
    ...
    NOTICE("Writing new storage metadata (version %llu) to %s",
           metadata.version(),
           filename.c_str());
                           ...
                           bool quiet) const
{
    ...
    if (error.empty()) {
        if (metadata.format_version() > 1) {
            PANIC("The format version found in %s is %llu but this code "
                  "only understands version 1",
                  filename.c_str(),
                  metadata.format_version());
        }
        NOTICE("Read metadata version %llu from %s",
...
SegmentedLog::loadClosedSegment(Segment& segment, uint64_t logStartIndex)
{
    ...
    if (segment.endIndex < logStartIndex) {
        NOTICE("Removing closed segment whose entries are no longer "
               "needed (last index is %llu but log start index is %llu): %s",
               segment.endIndex,
               logStartIndex,
               segment.filename.c_str());
        FS::removeFile(dir, segment.filename);
    ...
    for (uint64_t index = segment.startIndex;
        ...
        if (!error.empty()) {
            PANIC("Could not read entry %llu in log segment %s "
                  "(offset %llu bytes). This indicates the file was "
    ...
    if (offset < reader.getFileLength()) {
        WARNING("Found an extra %llu bytes at the end of closed segment "
    ...
    while (offset < reader.getFileLength()) {
        ...
        if (!error.empty()) {
            ...
            if (isAllZeros(reader.get(offset, remainingBytes),
                           remainingBytes)) {
                WARNING("Truncating %llu zero bytes at the end of log "
                        "segment %s (%llu bytes into the segment, "
                        "following  entry %llu). This is most likely "
                ...
                WARNING("Could not read entry in log segment %s "
                        "(%llu bytes into the segment, following "
                        "entry %llu), probably because it was being "
                        "written when the server crashed. Discarding the "
                        "remainder of the file (%llu bytes). Error was: %s",
                        segment.filename.c_str(),
                        offset,
                        lastIndex,
                        remainingBytes,
...
SegmentedLog::closeSegment()
{
    ...
    if (openSegment.startIndex > openSegment.endIndex) {
        ...
        NOTICE("Removing empty open segment (start index %llu): %s",
