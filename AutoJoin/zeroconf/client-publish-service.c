...
static AvahiSimplePoll *publish_simple_poll = NULL;
...
static void entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state, AVAHI_GCC_UNUSED void *userdata)
{
    ...
    case AVAHI_ENTRY_GROUP_FAILURE:
        ...
        avahi_simple_poll_quit(publish_simple_poll);
        break;
...
int publish(bool fork_program)
{
    ...
    if (!(publish_simple_poll = avahi_simple_poll_new()))
    {
        fprintf(stderr, "Failed to create simple poll object.\n");
        goto fail;
    }

    name = avahi_strdup("Host");

    /* Allocate a new client */
    client = avahi_client_new(avahi_simple_poll_get(publish_simple_poll), 0, publish_client_callback, NULL, &error);
    ...
    // avahi_simple_poll_get(publish_simple_poll)->timeout_new(avahi_simple_poll_get(publish_simple_poll), avahi_elapse_time(&tv, 1000 * 10, 0), modify_callback, client);
    ...
    //     pid = fork();
    // }
    // if (!fork_program || (fork_program && (pid == 0)))
    // {
    //     cout << "pid: " << pid << std::endl;

    avahi_simple_poll_loop(publish_simple_poll);
    // }
    // else
    ret = 0;
fail:
    ...
    if (publish_simple_poll)
        avahi_simple_poll_free(publish_simple_poll);
